<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            $check = mysql_query("SELECT * FROM grandballroom WHERE id != '$id'");
            while($checkrow = mysql_fetch_array($check))
            {
               if($checkrow['gbrName'] == $functionName)
               {
                   $error = true;
                   break;
               }
            ...
            if($error == true)
            {
                echo '<script>alert("Room Already Exist!"); location.href="update-grandball.php?id='.$id.'";</script>';
            }
            else
            {
               mysql_query("UPDATE grandballroom SET gbrName = '$functionName', gbrRate = '$functionRate' , perHour = '$functionPerHour' WHERE id = '$id'");
               mysql_close();
               header("Location: index-grandball.php"); 
            }
        ...
        $functionName = $row['gbrName'];
        $functionRate = $row['gbrRate'];
...
<body>
    <div class="container">
                ...
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="update-grandball.php?id=<?php echo $id?>" method="post">
