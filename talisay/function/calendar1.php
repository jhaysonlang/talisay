...
$name = '';
$roomType = $_GET['type'];
$charot = mysql_query("SELECT * FROM cottagecategory WHERE cottageType = '$roomType'");
...
while($endday >= $daysctr)
{
    ...
    {
      echo '<th style ="background:green;">'.$daysctr.'<br> Available Cottages:' . $numcat.'</th>';
    }
    }
	if($ctr % 7 == 0)
	{
...
$(document).ready(function(){
	$('.prev').on('click',function(e){
		...
		var type = $('.type').val();
      	location.href="calendar1.php?prev&month="+month+"&year="+year+"&type="+type;
	});
	...
	$('.next').on('click',function(e){
		...
		var year = $('.prev').data('year');
		var type = $('.type').val();
		location.href="calendar1.php?next&month="+month+"&year="+year+"&type="+type;
        $.ajax({
          type: 'POST',
          url: "next.php",
