<?php 
    ...
    if ( !empty($_POST)) {
        ...
        // insert data
        if ($valid) {
            $check = mysql_query("SELECT * FROM grandballroom WHERE gbrName = '$functionName'");
            if(mysql_num_rows($check) > 0)
            {
                echo '<script>alert("Room Already Exist!"); location.href="create-grandball.php";</script>';
            }
            else
            {
            mysql_query("INSERT INTO grandballroom (gbrName,gbrRate,perHour) values('$functionName', '$functionRate', '$functionPerHour')");
            mysql_close();
